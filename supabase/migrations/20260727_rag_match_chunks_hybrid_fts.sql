-- Hybrid match_chunks: vector cosine + Postgres full-text (portuguese).
-- Score = 0.7 * similarity + 0.3 * fts_norm  when match_query is provided;
-- otherwise score = similarity (backward compatible).
-- fts_norm = ts_rank / (ts_rank + 1)  → soft-normalize into (0, 1).
-- Filters unchanged: group, tenant, modalidade, bairro, plano_rank, municipio.
-- Optional match_query (default null) — pass the user question for FTS boost.
...
-- FTS index on chunk text (portuguese config)
...
comment on function public.match_chunks(
  ...
  'Hybrid retrieval: score = 0.7*cosine_similarity + 0.3*(ts_rank/(ts_rank+1)) when match_query set; else vector-only. Filters: modalidade/bairro/plano_rank/municipio.';
