...
import { 
  ...
  FileText, Phone, Mail, Paperclip, Send, CheckSquare, Clock, ShieldAlert
} from 'lucide-react';
...
const Kanban: React.FC = () => {
  ...
  const canEdit = hasPermission('edit_kanban'); // true for admin & creator
  ...
  return (
    <div className="h-full flex flex-col bg-slate-950 text-slate-50 p-6 overflow-hidden relative">
      ...
      {!canEdit && (
        <div className="mb-6 mx-2 p-4 rounded-xl bg-amber-950/40 border border-amber-800/40 text-sm text-amber-300 flex items-center gap-3 flex-shrink-0 animate-in fade-in duration-300">
          <ShieldAlert className="w-5 h-5 text-amber-400 flex-shrink-0 animate-pulse" />
          <p>
            <strong>Acesso de Leitura:</strong> Como <strong className="text-white">{getRoleName()}</strong>, você possui autorização apenas para visualizar os negócios ativos. O arrastar-e-soltar e a criação de novos Deals estão desabilitados.
          </p>
        </div>
      )}
